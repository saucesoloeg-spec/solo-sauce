        ...
        <div class="card border shadow-xs mb-4">
            <div class="card-header border-bottom pb-0">
                <div class="d-sm-flex align-items-center">
                    <div>
                        <h6 class="font-weight-semibold text-lg mb-0">{{ __('drivers.drivers_list') }}</h6>
                        <p class="text-sm">{{ __('drivers.drivers_list_description') }}</p>
                    </div>
                    <div class="d-flex align-items-center @if(app()->getLocale() == 'ar') me-auto @else ms-auto @endif" style="gap: 10px;">
                        <a href="{{ route('drivers.create') }}" class="btn btn-outline-secondary btn-sm d-flex align-items-center" style="gap: 5px; white-space: nowrap;">
            ...
            <div id="delete-modal" class="modal">
                <div class="modal-content">
                    <h3>{{ __('drivers.confirm_deletion') }}</h3>
                    <p>{{ __('drivers.delete_confirmation_message') }}</p>
                    <div class="modal-buttons">
                        <button id="confirm-delete" class="btn-confirm">{{ __('drivers.confirm') }}</button>
                        <span id="loader" class="loader" style="display: none;"></span>
                        <button id="cancel-delete" class="btn-cancel">{{ __('drivers.cancel') }}</button>
                    </div>
                </div>
            </div>
            <!-- Confirmation Modal -->
            <div class="card-body px-0 py-0">
                <div class="border-bottom py-3 px-3 d-sm-flex align-items-center">
                    <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                        ...
                        <label class="btn btn-white px-3 mb-0" for="filter-all">{{ __('drivers.filter_all') }}</label>
                        <input id="filter-verified" type="radio" class="btn-check" name="btnradiotable" autocomplete="off">
                        <label class="btn btn-white px-3 mb-0" for="filter-verified">{{ __('drivers.filter_verified') }}</label>
                        <input id="filter-pending" type="radio" class="btn-check" name="btnradiotable" autocomplete="off">
                        <label class="btn btn-white px-3 mb-0" for="filter-pending">{{ __('drivers.filter_pending') }}</label>
                    </div>
                    ...
                    <div class="input-group w-sm-25 @if(app()->getLocale() == 'ar') me-auto @else ms-auto @endif" style="display: flex; gap: 10px;">
                        ...
                        <input type="text" class="form-control" id="searchInput" placeholder="{{ __('drivers.search_by_name') }}" aria-label="Search by name">
                    </div>
                </div>
                <div class="table-responsive p-0">
                    <table class="table align-items-center mb-0" id="driversTable">
                        <thead class="bg-gray-100">
                            <tr>
                                <th class="text-secondary text-xs font-weight-semibold opacity-7">{{ __('drivers.driver') }}</th>
                                <th class="text-secondary text-xs font-weight-semibold opacity-7 ps-2">{{ __('drivers.email') }}</th>
                                <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">{{ __('drivers.phone') }}</th>
                                <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">{{ __('drivers.address') }}</th>
                                <th class="text-center text-secondary text-xs font-weight-semibold opacity-7">{{ __('drivers.registered_at') }}</th>
                                <th class="text-secondary opacity-7"></th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($drivers as $key => $driver)
                            <tr data-status="{{ $driver->email_verified_at ? 'verified' : 'pending' }}" data-driver-id="{{ $driver->id }}" id="row-{{$driver->id}}">
                                <td>
                                    <div class="d-flex px-2 py-1">
                                        ...
                                        <div class="d-flex flex-column justify-content-center ms-1">
                                            <h6 class="mb-0 text-sm font-weight-semibold">{{ $driver->name }}</h6>
                                            <!-- <p class="text-sm text-secondary mb-0">{{ $driver->email }}</p>
                                            <p class="text-sm text-secondary mb-0">{{ $driver->phone_number }}</p> -->
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $driver->email }}</p>
                                </td>
                                <td class="text-center">
                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $driver->phone }}</p>
                                </td>
                                <td class="text-center">
                                    <p class="text-sm text-dark font-weight-semibold mb-0">{{ $driver->address }}</p>
                                </td>
                                <td class="align-middle text-center">
                                    <span class="text-secondary text-sm font-weight-normal">{{ $driver->created_at->toDateString() }}</span>
                                </td>
                                <td class="align-middle">
                                    <a href="{{ route('drivers.show', ['id' => $driver->id]) }}" class="text-secondary font-weight-bold text-xs m-2 view cursor-pointer" data-bs-toggle="tooltip" data-bs-title="{{ __('drivers.view_driver') }}">
                                    ...
                                    <a href="javascript:;" class="text-secondary font-weight-bold text-xs m-2 delete cursor-pointer" data-bs-toggle="tooltip" data-bs-title="{{ __('drivers.delete_sale') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="border-top py-3 px-3 d-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        <button class="btn btn-sm btn-white mb-0 previous">{{ __('drivers.previous') }}</button>
                        <button class="btn btn-sm btn-white mb-0 next">{{ __('drivers.next') }}</button>
                    </div>
                </div>
            </div>
        </div>
